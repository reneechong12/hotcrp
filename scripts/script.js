...
var siteurl, siteurl_postvalue, siteurl_suffix, siteurl_defaults, siteurl_absolute_base, siteurl_cookie_params, assetsurl, hotcrp_paperid, hotcrp_status, hotcrp_user, hotcrp_pc, hotcrp_want_override_conflict;
function $$(id) {
...
function count_words_split(text, wlimit) {
    var re = new RegExp("^((?:[-\\s.,;:<>!?*_~`#|]*[^-\\s.,;:<>!?*_~`#|]\\S*(?:\\s|$)\\s*){" + wlimit + "})([\\d\\D]*)$"), m = re.exec(text || "");
    return m ? [m[1], m[2]] : [text || "", ""];
}
function sprintf(fmt) {
    var words = fmt.split(/(%(?:%|-?(?:\d*|\*?)(?:\.\d*)?[sdefgoxX]))/), wordno, word, arg, argno, conv, pad, t = "";
    for (wordno = 0, argno = 1; wordno != words.length; ++wordno) {
        ...
        if (word.charAt(0) != "%")
            ...
            if (conv[4] == "g") {
                ...
                arg = arg.replace(/\.(\d*[1-9])?0+(|e.*)$/, function (match, p1, p2) {
...
var event_key = (function () {
    var key_map = { "Spacebar": " ", "Esc": "Escape" }, charCode_map = { "9": "Tab", "13": "Enter", "27": "Escape" }, keyCode_map = {
...
handle_ui.on("js-range-click", function (event) {
    ...
    if ($f[0].hasAttribute("data-range-clicking")
        ...
        return;
    var kind = this.getAttribute("data-range-type"), kindsearch = kind ? "[data-range-type~='" + kind + "']" : "[name='" + this.name + "']", $cbs = $f.find("input[type=" + this.type + "]" + kindsearch);
...
var make_bubble = (function () {
    var capdir = ["Top", "Right", "Bottom", "Left"], lcdir = ["top", "right", "bottom", "left"], szdir = ["height", "width"], SPACE = 8;
    ...
    return function (content, bubopt) {
        if (!bubopt && content && typeof content === "object") {
            ...
            bubopt = { color: bubopt };
        var nearpos = null, dirspec = bubopt.dir, dir = null, color = bubopt.color ? " " + bubopt.color : "";
        ...
        function constrainmid(nearpos, wpos, ds, ds2) {
            var z0 = nearpos[lcdir[ds]], z1 = nearpos[lcdir[ds ^ 2]], z = (1 - ds2) * z0 + ds2 * z1;
        ...
        function constrain(za, wpos, bpos, ds, ds2, noconstrain) {
            var z0 = wpos[lcdir[ds]], z1 = wpos[lcdir[ds ^ 2]], bdim = bpos[szdir[ds & 1]], z = za - ds2 * bdim;
        ...
        function bpos_wconstraint(wpos, ds) {
            var xw = Math.max(ds === 3 ? 0 : nearpos.left - wpos.left, ds === 1 ? 0 : wpos.right - nearpos.right);
        ...
        function show() {
            ...
            if (dirspec == null)
                dirspec = "r";
            var noflip = /!/.test(dirspec), noconstrain = /\*/.test(dirspec), dsx = dirspec.replace(/[^a0-3neswtrblhv]/, ""), ds = parse_dirspec(dsx, 0), ds2 = parse_dirspec(dsx, 1);
...
window.hotcrp_deadlines = (function ($) {
    ...
    function display_main(is_initial) {
        ...
        var s = "", i, x, subtype, browser_now = now_sec(), now = +dl.now + (browser_now - +dl.load), elt = $$("maindeadline");
        if (!elt)
            return;
        if (!is_initial
    ...
    // tracker
    var has_tracker, had_tracker_at, last_tracker_html, tracker_has_format, tracker_timer, tracker_refresher;
    ...
    function tracker_show_elapsed() {
        ...
        tracker_timer = setTimeout(tracker_show_elapsed, 1000 - (delta * 1000) % 1000);
    }
    ...
    function display_tracker() {
        var mne = $$("tracker"), mnspace = $$("trackerspace"), mytracker = is_my_tracker(), body, t, tt, i, e;
        // tracker button
    ...
    // Comet tracker
    var comet_sent_at, comet_stop_until, comet_nerrors = 0, comet_nsuccess = 0, comet_long_timeout = 260000;
    ...
    function comet_tracker() {
        var at = now_msec(), timeout = Math.floor((comet_nsuccess ? comet_long_timeout : 1000)
            + Math.random() * 1000);
        ...
        $.ajax(hoturl_add(dl.tracker_site + "poll", "conference=" + encodeURIComponent(hoturl_absolute_base())
...
handle_ui.on("js-aufoldup", function (event) {
    var e = $$("foldpaper"), m9 = e.className.match(/\bfold9([co])\b/), m8 = e.className.match(/\bfold8([co])\b/);
    if (m9 && (!m8 || m8[1] == "o"))
...
$(document).on("keypress", "input.js-autosubmit", function (event) {
    if (event_modkey(event) || event_key(event) !== "Enter")
        return;
    var $f = $(event.target).closest("form"), type = $f.data("autosubmitType"), defaulte = $f[0] ? $f[0]["default"] : null;
    if (defaulte && type) {
...
(function ($) {
    function make_radio(name, value, html, revtype) {
        var rname = "assrev" + name, id = rname + "_" + value, t = '<div class="assignment-ui-choice">'
    ...
    function close_unnecessary(event) {
        var $a = $(event.target).closest(".has-assignment"), $as = $a.closest(".has-assignment-set"), d = $as.data("lastAssignmentModified");
    ...
    handle_ui.on("js-assignment-fold", function (event) {
        var $a = $(event.target).closest(".has-assignment"), $x = $a.find(".has-assignment-ui");
        if ($a.hasClass("foldc")) {
            setup($a);
            // close_unnecessary(event);
            if (!$x.length) {
                var name = $a.attr("data-pid") + "u" + $a.attr("data-uid"), revtype = +$a.attr("data-review-type"), conftype = +$a.attr("data-conflict-type"), revinprogress = $a[0].hasAttribute("data-review-in-progress");
...
var row_order_ui = (function ($) {
    function row_order_change(e, delta, action) {
        ...
        var max_rows = +$tbody.attr("data-max-rows") || 0, min_rows = Math.max(+$tbody.attr("data-min-rows") || 0, 1), autogrow = $tbody.attr("data-row-order-autogrow");
        if (action < 0) {
            $r.remove();
            delta = 0;
        }
    ...
    function row_order_ui(event) {
        if (hasClass(this, "moveup"))
            ...
            var $parent = $(this).closest(".js-row-order"), $child = $parent.children().filter("[data-row-template]");
...
window.render_text = (function ($) {
    ...
    function on() {
        var $self = $(this), format = this.getAttribute("data-format"), content = this.getAttribute("data-content") || $self.text(), args = null, f, i;
...
window.review_form = (function ($) {
        ...
        if (rv.length) {
            ...
            if (fieldj && (fieldj.description || fieldj.options)) {
                ...
                if (fieldj.options) {
                    ...
                    for (si = 0, vo = fieldj.score_info.value_order(); si < vo.length; ++si)
    ...
    handle_ui.on("js-revrating", function () {
        ...
        $.post(hoturl_post("api", { p: $card.attr("data-pid"), r: $card.data("rid"),
            fn: "reviewrating" }), { user_rating: (current & ~off) | on }, function (data, status, jqxhr) {
    ...
    function add_review(rrow) {
        var hc = new HtmlCollector, rid = rrow.ordinal ? rrow.pid + "" + rrow.ordinal : "" + rrow.rid, rlink = "r=" + rid + (hotcrp_want_override_conflict ? "&forceShow=1" : ""), has_user_rating = false, i, ratekey, selected;
...
window.papercomment = (function ($) {
    ...
    function visibility_change() {
        var j = $(this).closest(".cmteditinfo"), dofold = j.find("select[name=visibility]").val() != "au";
        fold(j[0], dofold, 2);
    }
    ...
    function render_cmt($c, cj, editing, msg) {
        ...
        if (editing)
            ...
            render_cmt_text(cj.format, cj.text || "", cj.response, $c.find(".cmttext"), chead);
        }
        return $c;
    }
    function render_cmt_text(format, value, response, textj, chead) {
        var t = render_text(format, value), wlimit, wc, fmt = "format" + (t.format || 0);
        textj.addClass(fmt);
...
tooltip.add_builder("votereport", function (info) {
    var pid = $(this).attr("data-pid") || hotcrp_paperid, tag = $(this).attr("data-tag");
    if (pid && tag)
...
var search_completion = new HPromise().onThen(function (search_completion) {
    jQuery.get(hoturl("api/searchcompletion"), null, function (v) {
        var sc = (v && v.searchcompletion) || [], scs = $.grep(sc, function (x) { return typeof x === "string"; }), sci = $.grep(sc, function (x) { return typeof x === "object"; }), result = [];
...
function suggest(elt, suggestions_promise, options) {
    ...
    function finish_display(cinfo) {
        ...
        var t = '<div class="suggesttable suggesttable' + (i + 1) +
            '">' + cinfo.list.join('') + '</div>';
        var $elt = jQuery(elt), shadow = textarea_shadow($elt, elt.tagName == "INPUT" ? 2000 : 0);
    ...
    function move_active(k) {
        var $sug = hintdiv.self().find(".suggestion"), $active = hintdiv.self().find(".suggestion.active"), pos = null;
        if (!$active.length) {
            if (k === "ArrowUp")
                pos = -1;
            else if (k === "ArrowDown")
            ...
            var $activeg = $active.geometry(), nextadx = Infinity, nextady = Infinity, isleft = k === "ArrowLeft", side = (isleft ? "left" : "right");
            for (var i = 0; i != $sug.length; ++i) {
                var $thisg = $($sug[i]).geometry(), dx = $activeg[side] - $thisg[side], adx = Math.abs(dx), ady = Math.abs(($activeg.top + $activeg.bottom) - ($thisg.top + $thisg.bottom));
                if ((isleft ? dx > 0 : dx < 0)
...
function setfollow() {
    var $self = $(this);
    $.post(hoturl_post("api/follow", { p: $self.attr("data-pid") || hotcrp_paperid }), { following: this.checked, reviewer: $self.data("reviewer") || hotcrp_user.email }, function (rv) {
...
window.plinfo_tags = (function () {
    ...
    function search_sort_success(tbl, data_href, data) {
        ...
        var want_sorter = data.fwd_sorter || href_sorter(data_href) || "id", want_fwd_sorter = want_sorter && sorter_toggle_reverse(want_sorter, false);
        var $sorters = $(tbl).children("thead").find("a.pl_sort");
        $sorters.removeClass("pl_sorting_fwd pl_sorting_rev")
            .each(function () {
            var href = this.getAttribute("href"), sorter = sorter_toggle_reverse(href_sorter(href), false);
    ...
    PaperRow.prototype.titlehint = function () {
        var tg = $(this.tbody.childNodes[this.l]).find("a.ptitle, span.plheading-group"), titletext = null, m;
        if (tg.length) {
    ...
    var plt_tbody, full_ordertag, dragtag, full_dragtag, rowanal, highlight_entries, dragging, srcindex, dragindex, dragger, scroller, mousepos, scrolldelta;
    ...
    function tag_save() {
        ...
        $.post(hoturl_post("api/settags", { p: m[2], forceShow: 1 }), { addtags: ch }, make_tag_save_callback(this));
    }
    ...
    function tag_scroll() {
        var geometry = $(window).geometry(), delta = 0, y = mousepos.clientY + geometry.top;
        if (y < geometry.top - 5)
    ...
    function tag_mousemove(evt) {
        ...
        var rows = plt_tbody.childNodes, geometry = $(window).geometry(), a, x = evt.clientX + geometry.left, y = evt.clientY + geometry.top;
    ...
    function rowcompar(a, b) {
        ...
        var at = $(rows[rowanal[a].l]).find(".plheading-group").attr("data-title"), bt = $(rows[rowanal[b].l]).find(".plheading-group").attr("data-title"), cmp = strnatcmp(at, bt);
        if (cmp)
            return cmp;
        else
    ...
    function row_move(srcindex) {
        ...
        if (dstindex !== srcindex + 1) {
            // shift row groups
            var range = [rowanal[srcindex].l, rowanal[srcindex].r], sibling, e;
    ...
    function commit_drag(si, di) {
        ...
        if (saves.length)
            $.post(hoturl_post("api/settags", { forceShow: 1 }), { tagassignment: saves.join(",") }, make_tag_save_callback(rowanal[si].entry));
        if (annosaves.length)
            $.post(hoturl_post("api/taganno", { tag: dragtag, forceShow: 1 }), { anno: JSON.stringify(annosaves) }, taganno_success);
    }
    ...
    function edit_anno(locator) {
        ...
        var mytag = elt.getAttribute("data-anno-tag"), annoid = elt.hasAttribute("data-anno-id") ? +elt.getAttribute("data-anno-id") : null;
        function clickh(evt) {
            if (this.name === "add") {
                var hc = new HtmlCollector;
                add_anno(hc, {});
                ...
                $.post(hoturl_post("api/taganno", { tag: mytag }), { anno: JSON.stringify(anno) }, make_onsave($d));
            }
            return false;
        }
...
function check_version(url, versionstr) {
    ...
    function updatecb(json) {
        if (json && json.updates && window.JSON)
            jQuery.get(siteurl + "checkupdates.php", { data: JSON.stringify(json) }, updateverifycb);
...
var plinfo = (function () {
    var self, fields, field_order, aufull = {}, tagmap = false, _bypid = {}, _bypidx = {};
    ...
    function render_allpref() {
        ...
        $(".need-allpref").each(function () {
            var t = [], m, pid = pidnear(this), allpref = pidattr(pid, "data-allpref") || "";
    ...
    function compute_row_tagset(ptr, tagstr) {
        ...
        for (var i = 0; i != tags.length; ++i) {
            ...
            if (text !== "" && (twiddle <= 0 || text.substr(0, twiddle) == hotcrp_user.cid)) {
                ...
                var tbase = text.substring(0, hash), tindex = text.substr(hash + 1), tagx = tmap ? tmap[tbase.toLowerCase()] || 0 : 0, h, q;
    ...
    function edittags_submit() {
        ...
        $.post(hoturl_post("api/settags", { p: pid, forceShow: 1 }), { tags: $(div).find("textarea").val() }, function (rv) {
            if (rv.ok)
    ...
    function add_column(f) {
        ...
        var classes = (f.className || 'pl_' + f.name) + ' fx' + f.foldnum, classEnd = ' class="pl ' + classes + '"', h = f.title, stmpl;
    ...
    function add_row(f) {
        var index = field_index(f), h = '<div class="' + (f.className || "pl_" + f.name) +
            " fx" + f.foldnum + '"></div>';
    ...
    function make_callback(dofold, type) {
        ...
        function render_statistics() {
            var tr = $(self).find("tfoot > tr.pl_statrow").first()[0], index = field_index(f), htmlk = f.name + ".stat.html";
            for (; tr; tr = tr.nextSibling)
                if (tr.nodeName === "TR" && tr.hasAttribute("data-statistic")) {
                    var stat = tr.getAttribute("data-statistic"), j = 0, td = tr.childNodes[index];
...
window.make_pattern_fill = (function () {
    var fmap = {}, cmap = { "whitetag": 1, "redtag": 2, "orangetag": 3, "yellowtag": 4, "greentag": 5, "bluetag": 6, "purpletag": 7, "graytag": 8 }, params = {
    ...
    return function (classes, class_prefix) {
        ...
        // create pattern
        var param = params[class_prefix] || params[""], id = "svgpat__" + canonical_index.replace(/\s+/g, "__"), size = param.size + Math.max(0, tags.length - 2) * param.incr, sw = size / tags.length, t = "";
        for (var i = 0; i < tags.length; ++i) {
            var x = $('<div class="' + class_prefix + tags[i] + '"></div>').appendTo(document.body), color = x.css(param.css);
            x.remove();
...
handle_ui.on("js-check-format", function () {
    var $self = $(this), $d = $self.closest(".has-document"), $cf = $d.find(".document-format");
...
handle_ui.on("js-check-submittable", function (event) {
    var $f = $(this).closest("form"), readye = $f[0].submitpaper, was = $f.attr("data-submitted"), is = true;
...
handle_ui.on("js-add-attachment", function () {
    var $ea = $($$(this.getAttribute("data-editable-attachments"))), $ei = $ea.find(".has-document"), $f = $ea.closest("form"), name, n = 0;
    do {
        ++n;
...
handle_ui.on("js-replace-document", function (event) {
    var $ei = $(this).closest(".has-document"), $u = $ei.find(".document-uploader");
    $ei.find(".document-remover").val("");
    if (!$u.length) {
        var docid = +$ei.attr("data-dtype"), name = docid > 0 ? "opt" + docid : "paperUpload", t = '<div class="document-upload hidden"><input id="' + name + '" type="file" name="' + name + '"';
...
handle_ui.on("js-remove-document", function (event) {
    var $ei = $(this).closest(".has-document"), $r = $ei.find(".document-remover"), $en = $ei.find(".document-file"), $f = $(this).closest("form") /* set before $ei is removed */;
...
handle_ui.on("js-withdraw", function (event) {
    var $f = $(this).closest("form"), hc = popup_skeleton({
...
handle_ui.on("js-clickthrough", function (event) {
    var self = this, $container = $(this).closest(".js-clickthrough-container");
    $.post(hoturl_post("api/clickthrough", { accept: 1 }), $(this).closest("form").serialize(), function (data) {
        if (data && data.ok) {
...
var edit_paper_ui = (function ($) {
    ...
    function prepare_psedit(url) {
        var self = this, $ctl = $(self).find("select, textarea").first(), val = $ctl.val();
        function cancel() {
            $ctl.val(val);
        ...
        function change() {
            var saveval = $ctl.val();
            $.post(hoturl_post("api", url), $(self).find("form").serialize(), function (data) {
                if (data.ok) {
                    done(true);
                    foldup.call(self, null, { f: true });
                    val = saveval;
    ...
    function prepare_pstags() {
        var self = this, $f = this.tagName === "FORM" ? $(self) : $(self).find("form"), $ta = $f.find("textarea");
        ...
        $(window).on("hotcrptags", function (evt, data) {
            if (data.pid == $f.attr("data-pid")) {
                var h = data.tags_view_html == "" ? "None" : data.tags_view_html, $p = $(self).find(".js-tag-result").first();
                if ($p.html() !== h)
                    $p.html(h);
                if ($ta.length
    ...
    function save_pstagindex(event) {
        ...
        $.post(hoturl_post("api/settags", { p: $f.attr("data-pid") }), { "addtags": tags.join(" ") }, done);
    }
    ...
    function run_edit_conditions() {
        $(".has-edit-condition").each(function () {
            var $f = $(this).closest("form"), ec = JSON.parse(this.getAttribute("data-edit-condition"));
...
if (hotcrp_paperid) {
    $(window).on("hotcrptags", function (event, data) {
        ...
        $(".is-tag-index").each(function () {
            var $j = $(this), res = "", t = $j.data("tagBase") + "#", i;
            if (t.charAt(0) == "~" && t.charAt(1) != "~")
                t = hotcrp_user.cid + t;
...
handle_ui.on("js-delete-user", function (event) {
    var $f = $(this).closest("form"), hc = popup_skeleton({ anchor: this, action: $f[0].action }), x;
...
var profile_ui = (function ($) {
    return function (event) {
        if (hasClass(this, "js-role")) {
            var $f = $(this).closest("form"), pctype = $f.find("input[name=pctype]:checked").val(), ass = $f.find("input[name=ass]:checked").length;
...
handle_ui.on("js-decline-review", function () {
    var $f = $(this).closest("form"), hc = popup_skeleton({ anchor: this, action: $f[0].action });
...
handle_ui.on("js-edit-formulas", function () {
    ...
    function submit(event) {
        event.preventDefault();
        $.post(hoturl_post("api/namedformula"), $d.find("form").serialize(), function (data) {
            if (data.ok)
...
var paperlist_ui = (function ($) {
    ...
    paperlist_ui.prepare_tag_listaction = function () {
        ...
        function placttag() {
            var $t = $(this).closest(".linelink"), $ty = $t.find("select[name=tagfn]");
...
handle_ui.on("js-assign-review", function (event) {
    ...
    data["pcs" + m[2]] = value;
    $.post(hoturl_post("assign", { p: m[1], update: 1, ajax: 1 }), data, function (rv) {
        if (self.tagName === "SELECT")
...
(function ($) {
    ...
    function update_digest(info) {
        var add = typeof info === "string" ? 1 : 0, digests = wstorage.json(false, "list_digests") || [], found = -1, now = now_msec();
    ...
    function row_click(evt) {
        ...
        if (hasClass(this.parentElement.parentElement, "pltable-focus-checkbox")) {
            ...
            var $a = $(pl).find("a.pnum").first(), href = $a[0].getAttribute("href");
    ...
    $(function () {
        ...
        if (had_digests && hotcrp_paperid
            ...
            $(".quicklinks").each(function () {
                var $l = $(this).closest(".has-hotlist"), info = JSON.parse($l.attr("data-hotlist") || "null"), ids, pos;
                if (info
                    && info.ids
...
var scorechart = (function ($) {
    ...
    function setup_canvas(canvas, w, h) {
        var ctx = canvas.getContext("2d"), dpr = window.devicePixelRatio || 1, bspr = ctx.webkitBackingStorePixelRatio
            || ctx.mozBackingStorePixelRatio
            || ctx.msBackingStorePixelRatio
            || ctx.oBackingStorePixelRatio
            || ctx.backingStorePixelRatio || 1, r = dpr / bspr, nw = Math.ceil(w * r), nh = Math.ceil(h * r);
        canvas.width = nw;
        canvas.height = nh;
        if (dpr !== bspr) {
    ...
    function scorechart1_s2(sc, parent) {
        var canvas = document.createElement("canvas"), ctx, anal = analyze_sc(sc), cwidth = 64, cheight = 8, x, vindex, pos = 0, x1 = 0, x2;
    ...
    function make_input_autogrower($self) {
        ...
        return function (event) {
            ...
            if (!shadow) {
                ...
                shadow.css({ width: "auto", display: "table-cell", paddingLeft: (parseFloat(p.paddingRight) + parseFloat(p.paddingLeft) + parseFloat(p.borderLeftWidth) + parseFloat(p.borderRightWidth)) + "px" });
            ...
            ws = $self.css(["minWidth", "maxWidth"]);
            var outerWidth = Math.min(shadow.outerWidth(), $(window).width()), maxWidth = parseFloat(ws.maxWidth);
